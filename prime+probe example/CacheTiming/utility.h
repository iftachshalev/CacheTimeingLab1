...
void print_array(const char *name, uint64_t *arr) {
    printf("\n%s array: ", name);
...
void print_results(uint64_t* dram, uint64_t* l1, uint64_t* l2, uint64_t* l3) {
    ...
    print_array("L2", l2);
    print_array("L3", l3);
    print_array("DRAM", dram);

    printf("\nmemory type  :    L1   L2    L3    Mem   \n");
    printf("-----------------------------------------\n");
...
// loading all the given buffer n times (LINEAR) and so, evicting the the target_buffer from some cache level
void evict_from_Lx(volatile uint64_t *eviction_buffer, size_t size, int n) {
    int useless = 1;
    for (int j = 0; j < n; j++) {
